...
source "virtualbox-iso" "ol9vbox" {
  boot_command             = [
    "<esc>", "<wait>", "linux text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/ol9.ks", "<enter>"
  ]
  disk_size                = "20480"
  ...
  output_directory         = "output-virtualbox-iso-ol9vbox"
  sata_port_count          = "10"
...
source "qemu" "ol9qemu" {
  iso_url            = "file://REPLACE_ME_INSTALL_ISO"
  iso_checksum       = "sha256:REPLACE_ME_SHA256SUM"
  output_directory   = "output-qemu-ol9qemu"
  shutdown_command   = "echo 'packer' | sudo -S shutdown -P now"
  disk_size          = "20480"
  format             = "qcow2"
  accelerator        = "kvm"
  ...
  machine_type       = "q35"
  boot_command       = ["<esc>", "<wait>", "linux text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/ol9.ks", "<enter>"]
  qemuargs           = [
    [ "-m", "1024M" ],
    ["-cpu", "host,+nx"]
  ]
...
build {
  ...
  provisioner "ansible" {
    playbook_file = "ansible/vagrant-OracleLinux-9.yml"
    user          = "vagrant"
